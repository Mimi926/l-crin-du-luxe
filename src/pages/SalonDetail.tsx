    ...
    return (
      <div className="min-h-screen bg-background flex items-center justify-center">
        <div className="text-center">
          ...
          <Link to="/salons">
            <Button variant="gold">Retour aux salons</Button>
          </Link>
        </div>
      </div>
    );
      ...
      <section className="pt-20">
        <div className="relative h-[50vh] md:h-[60vh] overflow-hidden">
          <motion.img
            ...
            src={salon.photos[selectedPhoto]}
            alt={salon.name}
                ...
                {salon.photos.map((photo, i) => (
                  <button
                    ...
                    className={`w-16 h-16 md:w-20 md:h-20 rounded-sm overflow-hidden border-2 transition-all ${
                      i === selectedPhoto ? "border-gold" : "border-cream/30"
                    }`}
                  >
        ...
        <div className="container mx-auto px-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
            ...
            <div className="lg:col-span-2 space-y-10">
              ...
              >
                <h2 className="font-display text-2xl font-semibold mb-6 gold-underline pb-2">
                  Services & Tarifs
                </h2>
                <div className="space-y-3">
                  {salon.services.map((service) => (
                    <div
                      key={service.id}
                      className="bg-card rounded-sm p-5 flex flex-col md:flex-row md:items-center justify-between gap-3 shadow-card hover:shadow-elevated transition-shadow"
                    >
                      <div className="flex-1">
                        <div className="flex items-center gap-3 mb-1">
                          <h3 className="font-display text-lg font-semibold">{service.name}</h3>
                          <span className="text-xs bg-secondary text-secondary-foreground px-2 py-0.5 rounded-sm font-body">
                            {service.category}
                          </span>
                        </div>
            ...
            <aside>
              <div className="sticky top-24 space-y-6">
                ...
                <Button variant="gold" size="lg" className="w-full font-body tracking-wider">
                  Réserver maintenant
                </Button>
              </div>
            </aside>
